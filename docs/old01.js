...
function getNameAndYear(rawFileUrl) {
  // 1. Eliminar el prefijo "file://"
  let path = rawFileUrl.replace(/^file:\/+/, '');

  // 2. Decodificar caracteres codificados (como %20 -> espacio)
  path = decodeURIComponent(path);

  // 3. Extraer subcadena (índices 13 a 16 inclusive = JS substring(13,17))
  const pathSubstring = path.substring(13, 17);

  // 4. Obtener nombre de archivo
...
window.addEventListener('DOMContentLoaded', async () => {
  // const data = await ipc|Renderer.invoke('get-playlists');
  ...
  function createNode(node) {
    ...
    li.addEventListener('click', async (e) => {
      ...
      if (node.type === 'folder' && node.path) {
        // const songs = await ipc|Renderer.invoke('get-songs', node.path);
...
function crearVideoPlayer(url) {
    ...
    // Verificar si ya existe un videoPlayer y eliminarlo
...
function destruirVideoPlayer() {
    // Obtener el contenedor y el video
    const container = document.getElementById('videoContainer');
